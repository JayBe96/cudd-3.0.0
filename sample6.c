...
int main(int argc, char *argv[]) {
    ...
    x0 = Cudd_bddNewVar(gbm); /* Create a new BDD variable x0 */

    // Create the first term (¬x1 ∧ x0)
    DdNode *term1 = Cudd_bddAnd(gbm, Cudd_Not(x1), x0);

    // Create the second term (x2 ∧ ¬x0)
    DdNode *term2 = Cudd_bddAnd(gbm, x2, Cudd_Not(x0));

    // Create the final BDD (y = term1 OR term2)
    y = Cudd_bddOr(gbm, term1, term2);
    ...
    sprintf(filename, "./bdd/y_function.dot"); /* Write .dot filename to a string */
